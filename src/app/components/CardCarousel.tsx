...
export default function CardCarousel({ events, carousel = false }: CardCarouselProps) {
  ...
  return (
  <div className="relative w-full px-0 sm:px-4 py-8 sm:py-16">
      {carousel ? (
        <div className="flex items-center">
          ...
          {/* Events Scrollable Row (Carousel) */}
          <div
            ref={scrollRef}
        ...
        <div
          className="flex flex-wrap gap-4 sm:gap-6 w-full relative"
        >
          {events.map((event, idx) => (
            <div
              key={idx}
              className={
                `${sportySectionTheme.card.className} m-2 w-full sm:w-1/2 md:w-1/2 lg:w-1/2 min-w-[220px]`
              }
            ...
            >
              <Image
                ...
                width={500}
                height={220}
                className="object-contain w-full rounded-t-xl bg-black"
                style={{ objectFit: "contain", width: '100%', height: '220px', backgroundColor: '#000' }}
              />
              <h3 className="font-extrabold text-white mt-3 mb-1 sm:mt-4 sm:mb-2 text-2xl uppercase tracking-wider" style={{fontFamily: sportySectionTheme.font.title.style.fontFamily}}>
                {event.title}
              </h3>
